...
class TrustworthyNet_classfier(nn.Module):
    def __init__(self, nfeats, n_view, n_classes, n, args, device):
        ...
        self.blocks = nn.ModuleList([Block(n_classes, nfeats[i], args.alpha, device)  for i in range(n_view)])
    ...
    def forward(self, features, lap, active):
        ...
        for i in range(0, self.block):
            for j in range(0, self.n_view):
                h[j] = self.blocks[j](E_list[-1][j], lap[j], features[j] / 1.0)
                if active == 'l1':
                    output_z.append(self.self_active_l1((h[j])))  # # evi_v
                    output_z_1.append(self.self_active_l1((h[j])) + 1)  # # a_v

                elif active == "l21":
                    output_z.append(self.self_active_l21(h[j]))  # # evi_v
                    output_z_1.append(self.self_active_l21(h[j]) + 1)  # # a_v
            E_list.append(output_z)  # # evi_v
            A_list.append(output_z_1)  # a_v
